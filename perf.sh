...
IPC_V0="$(
    grep 'cpu_core/instructions/u' perfs/perfs_v0.txt |
    awk '{print $4}' |
    tr ',' '.' |
    awk '{sum += $1; n++} END {printf sum/n}'
)"
IPC_V1="$(
    grep 'cpu_core/instructions/u' perfs/perfs_v1.txt |
    awk '{print $4}' |
    tr ',' '.' |
    awk '{sum += $1; n++} END {printf sum/n}'
)"
IPC_V1B="$(
    grep 'cpu_core/instructions/u' perfs/perfs_v1b.txt |
    awk '{print $4}' |
    tr ',' '.' |
    awk '{sum += $1; n++} END {printf sum/n}
')"
IPC_V2="$(
    grep 'cpu_core/instructions/u' perfs/perfs_v2.txt |
    awk '{print $4}' |
    tr ',' '.' |
    awk '{sum += $1; n++} END {printf sum/n}'
)"
...
CM_V0="$(
    grep 'cpu_core/branch-misses/u' perfs/perfs_v0.txt |
    awk '{print $4}'|
    tr ',' '.' |
    awk '{sum += $1; n++} END {printf sum/n}'
 )"
CM_V1="$(
    grep 'cpu_core/branch-misses/u' perfs/perfs_v1.txt |
    awk '{print $4}'|
    tr ',' '.' |
    awk '{sum += $1; n++} END {printf sum/n}'
 )"
CM_V1B="$(
    grep 'cpu_core/branch-misses/u' perfs/perfs_v1b.txt |
    awk '{print $4}' |
    tr ',' '.' |
    awk '{sum += $1; n++} END {printf sum/n}'
)"
CM_V2="$(
grep 'cpu_core/branch-misses/u' perfs/perfs_v2.txt |
    awk '{print $4}' |
    tr ',' '.' |
    awk '{sum += $1; n++} END {printf sum/n}'
 )"
...
Branche-miss rate|${CM_V0}|${CM_V1}|${CM_V1B}|${CM_V2}\n" | column -t -s '|' > results/perfs.txt
